import re
import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import accuracy_score, classification_report

from joblib import dump
import sys
import os

# Get the directory where the script is located
script_dir = os.path.dirname(os.path.abspath(__file__))

# Add the script directory to sys.path to find model_utils if running from root
if script_dir not in sys.path:
    sys.path.append(script_dir)

from rf_model_utils import tokenizer

try:
    train_df = pd.read_csv(os.path.join(script_dir, "rf_train_dataset.csv"))
    test_df = pd.read_csv(os.path.join(script_dir, "rf_test_dataset.csv"))
except FileNotFoundError:
    print("Error: Error in finding the dataset. Please run splitData.py first.")
    exit()

# Note: We do NOT manually apply tokenizer here anymore. 
# We pass the tokenizer to TfidfVectorizer and let it handle raw text.

vectorizer = TfidfVectorizer(
    analyzer='word',
    ngram_range=(1, 4),
    max_features=15000,
    min_df=3,
    # token_pattern is ignored when tokenizer is provided
    tokenizer=tokenizer
)

print("vectorizing...")
train_X = vectorizer.fit_transform(train_df['code_snippet'])
train_y = train_df['label']
test_X = vectorizer.transform(test_df['code_snippet'])
test_y = test_df['label']
print("vectorizing done")

#TRAINING THE MODEL
print("training...")
model = RandomForestClassifier(n_estimators=150, random_state=42)
model.fit(train_X, train_y)
print("training done")

#EVALUATION
print("evaluating...")
y_pred = model.predict(test_X)
print("evaluating done")

print("Accuracy:", accuracy_score(test_y, y_pred))
print("Classification Report:")
print(classification_report(test_y, y_pred))

# Save the model and vectorizer
print("Saving model...")
dump(model, os.path.join(script_dir, "rf_model.joblib"))
dump(vectorizer, os.path.join(script_dir, "vectorizer.joblib"))
print(f"Model saved to {os.path.join(script_dir, 'rf_model.joblib')}")
