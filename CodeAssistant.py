...
def process_file_or_input(user_input: str, predictor_model, repairer_model, context: str = ""):
    # ----------------------------------------------------------------
    ...
    for func_name, func_code in functions:
        ...
        is_buggy, confidence, bug_type = predictor_model.predict(func_code, threshold=0.30)

        if is_buggy:
            print(f"  [STATUS]: BUGGY — {bug_type} (Confidence: {confidence:.2%})")
            print(f"  [ACTION]: Repairing...")
            try:
                intention = bug_type or context
                fixed_code = repairer_model.fix(func_code, intention=intention)
            ...
            print(f"  [STATUS]: CLEAN (Confidence: {confidence:.2%})\n")
